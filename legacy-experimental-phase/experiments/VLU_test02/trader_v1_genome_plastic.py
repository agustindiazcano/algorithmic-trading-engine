...
def save_brain_state(brain, filename, metadata=None):
    ...
    Incluye energía, edad, y vault para preservar la experiencia.
    """
    brain_state = {
        'genome': brain.genome,
...
def visualize_neural_network(brain, save_path='neural_network_3d.png'):
    """
    Crea visualización 3D de la topología de la red neuronal.
    - Verde: Neuronas LONG (peso positivo)
    - Rojo: Neuronas SHORT (peso negativo)
    - Intensidad: Nivel de energía
    - Gris: Neuronas hibernadas (vault)
    """
    ...
    # Plotear neuronas activas
    for i, gene in enumerate(brain.genome):
        cx, cy, cz = gene[:3]
        r = gene[3] * brain.A_dc
        sx, sy, sz = gene[5:8]
        weight = gene[8]
        energy = brain.neuron_energy[i]
        
        # Generar superficie elipsoide
        u, v = np.mgrid[0:2*np.pi:10j, 0:np.pi:5j]
        x = cx + r*sx*np.cos(u)*np.sin(v)
        y = cy + r*sy*np.sin(u)*np.sin(v)
        z = cz + r*sz*np.cos(v)
        
        color = 'green' if weight > 0 else 'red'
        alpha = min(max(energy / 5.0, 0.1), 1.0)  # Transparencia basada en energía
        
        ax.plot_wireframe(x, y, z, color=color, alpha=alpha, linewidth=0.5)
    
    # Plotear vault (hibernadas)
    if len(brain.vault_genome) > 0:
        vault_centers = brain.vault_genome[:, :3]
        ax.scatter(vault_centers[:, 0], vault_centers[:, 1], vault_centers[:, 2], 
                   color='gray', s=20, alpha=0.3, marker='x', label='Hibernadas')
    
    ax.set_xlabel('Precio Normalizado')
    ax.set_ylabel('Volumen Normalizado')
    ax.set_zlabel('Volatilidad Normalizada')
    ax.set_title(f'Red Neuronal: {len(brain.genome)} activas, {len(brain.vault_genome)} hibernadas')
    ax.legend()
    
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    print(f"   🖼️  Visualización guardada: {save_path}")
    plt.close()
    
    # Estadísticas
    print(f"\n📊 Estadísticas de la Red Neuronal:")
    print(f"   Neuronas Activas: {len(brain.genome)}")
    print(f"   Neuronas Hibernadas: {len(brain.vault_genome)}")
    print(f"   Energía Promedio: {np.mean(brain.neuron_energy):.2f}")
    print(f"   Edad Promedio: {np.mean(brain.neuron_age):.0f} velas")
    print(f"   Veteranos (>2000): {np.sum(brain.neuron_age > 2000)}")
...
def run_evolution(generations=1000, population_size=10, candles_limit=10000): 
    ...
    # Mejor Genoma Global (y estado completo)
    best_global_genome = None
    best_global_fitness = -np.inf
    best_global_energy = None
    best_global_age = None
    best_global_vault_genome = None
    best_global_vault_ages = None
    ...
    for gen in range(generations):
        ...
        profitable_count = 0
        gen_stats_best = {}
        gen_best_brain = None  # Cerebro completo del campeón de esta gen
        ...
        for i in range(population_size):
            ...
            if final_balance > gen_best_fitness:
                gen_best_fitness = final_balance
                gen_best_genome = [np.copy(g) for g in learned_genome]
                gen_stats_best = stats
                # Guardar cerebro completo
                gen_best_brain = brain  # Ya tiene todo el estado
        ...
        if gen_best_fitness > best_global_fitness:
            ...
            # Guardar estado completo del cerebro
            if gen_best_brain is not None:
                best_global_energy = np.copy(gen_best_brain.neuron_energy)
                best_global_age = np.copy(gen_best_brain.neuron_age)
                best_global_vault_genome = np.copy(gen_best_brain.vault_genome)
                best_global_vault_ages = np.copy(gen_best_brain.vault_ages)
                best_global_brain = gen_best_brain
            ...
            # Crear cerebro temporal para guardar
            temp_brain = NeuroPlasticBrain(initial_neurons=10)
            temp_brain.genome = np.array(best_global_genome)
            temp_brain.neuron_energy = best_global_energy
            temp_brain.neuron_age = best_global_age
            temp_brain.vault_genome = best_global_vault_genome
            temp_brain.vault_ages = best_global_vault_ages
            
            save_brain_state(temp_brain, "best_brain_COMPLETE.npy", metadata)
...
if __name__ == "__main__":
    ...
    winner_brain = run_evolution(generations=10, population_size=50, candles_limit=10000)
    
    if winner_brain is not None:
        # Generar visualización de la red neuronal
        print("\n" + "="*60)
        print("🧠 GENERANDO VISUALIZACIÓN DE LA RED NEURONAL")
        print("="*60)
        
        # Si es un brain object, visualizar. Si es array (fallback), skip
        if hasattr(winner_brain, 'genome'):
            visualize_neural_network(winner_brain, 'best_brain_topology.png')
            winner_genome = winner_brain.genome
        else:
            # Fallback: es un array numpy
            winner_genome = winner_brain
            print("   ⚠️  No se pudo visualizar (cerebro no completo)")

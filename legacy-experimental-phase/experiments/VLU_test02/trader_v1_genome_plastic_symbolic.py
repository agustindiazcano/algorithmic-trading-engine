        ...
        for i in range(population_size):
            ...
            if i == 0 and gen > 0:
                 ...
                 if best_global_brain is not None:
                     brain.neuron_score = np.copy(best_global_brain.neuron_score)
                     brain.neuron_best_score = np.copy(best_global_brain.neuron_best_score)
                     brain.neuron_hits = np.copy(best_global_brain.neuron_hits)
                     brain.protected = np.copy(best_global_brain.protected)
                     
                     # Heredar vault completo
                     brain.vault_genome = np.copy(best_global_brain.vault_genome)
                     brain.vault_score = np.copy(best_global_brain.vault_score)
                     brain.vault_hits = np.copy(best_global_brain.vault_hits)
                     
                     # Heredar símbolos
                     brain.symbol_genome = np.copy(best_global_brain.symbol_genome)
                 else:
                ...
                if best_global_brain is not None and len(best_global_brain.genome) > 0:
                    old_size = len(best_global_brain.genome)
                    new_size = len(brain.genome)
                    
                    # Copiar datos existentes + extender con zeros si creció
                    if new_size > old_size:
                        brain.neuron_score = np.concatenate([best_global_brain.neuron_score, np.zeros(new_size - old_size)])
                        brain.neuron_best_score = np.concatenate([best_global_brain.neuron_best_score, np.zeros(new_size - old_size)])
                        brain.neuron_hits = np.concatenate([best_global_brain.neuron_hits, np.zeros(new_size - old_size, dtype=int)])
                        brain.protected = np.concatenate([best_global_brain.protected, np.zeros(new_size - old_size, dtype=bool)])
                    else:
                        # Si redujo, truncar
                        brain.neuron_score = np.copy(best_global_brain.neuron_score[:new_size])
                        brain.neuron_best_score = np.copy(best_global_brain.neuron_best_score[:new_size])
                        brain.neuron_hits = np.copy(best_global_brain.neuron_hits[:new_size])
                        brain.protected = np.copy(best_global_brain.protected[:new_size])

...
MIN_CLUSTER_SIZE = 3             # Mínimo neuronas para simbolizar (bajado para facilitar)
SYMBOL_RESOLUTION = 0.05         # Granularidad del grid espacial
...
class NeuroPlasticBrain:
    def __init__(self, initial_neurons=10, A_dc=1.618):
        ...
        # MOTOR SIMBÓLICO (Compresión de memoria)
    ...
    def predict(self, points):
        """
        Predicción híbrida: Neuronas vivas + Símbolos consolidados.
        Retorna señal y activaciones (solo de neuronas vivas para aprendizaje).
        """
        point = points[-1].reshape(1, 3)
        total_activation = 0
        activations = np.array([])
        
        # PASE 1: NEURONAS VIVAS (memoria a corto plazo)
        if len(self.genome) > 0:
            ...
            # Contribución de neuronas vivas
            total_activation += np.sum(activations * weights)
        
        # PASE 2: SÍMBOLOS CONSOLIDADOS (memoria a largo plazo)
        if len(self.symbol_genome) > 0:
            ...
            # Contribución de símbolos (reducida: memoria a largo plazo más difusa)
            total_activation += np.sum(sym_activations * sym_weights) * 0.5
        
        # Retornamos señal final Y activaciones de neuronas vivas (para aprendizaje)
    ...
    def consolidate_memory(self, verbose=False):
        ...
        for indices in clusters.values():
            ...
            neurons_to_remove.update(indices)
        ...
        # 4. Eliminar neuronas comprimidas
        ...
        if verbose:
            print(f"🧹 Consolidación: {len(self.symbol_genome)} símbolos | {len(self.genome)} neuronas activas")
    ...
    def _structural_plasticity(self):
        """Maneja mitosis (nacimiento) y apoptosis (muerte) con inmunidad por edad."""
        new_genome = []
        new_energy = []
        new_age = []
        ...
        for i in range(len(self.genome)):
            ...
            # --- APOPTOSIS (MUERTE) con Inmunidad ---
            # Jóvenes: inmunes (no mueren por baja energía)
            # Viejos: mueren si energy <= 0
            if energy <= 0 and not is_young and len(self.genome) - dead_count > self.MIN_NEURONS:
                dead_count += 1
        ...
        if len(new_genome) > self.MAX_NEURONS:
            ...
            sorted_indices = np.argsort(energies)  # Del más débil al más fuerte
            to_kill = len(new_genome) - self.MAX_NEURONS
            
            # Mantener solo los más fuertes
            keep_indices = sorted_indices[to_kill:]
...
def fetch_extended_data(symbol="PEPEUSDT", interval="1m", total_candles=100000, custom_end_time=None):
    """
...
def save_brain_state(brain, filename, metadata=None):
    ...
    Incluye energía, edad, vault, símbolos, y decision_threshold para preservar la experiencia.
    """
    brain_state = {
        'genome': brain.genome,
        ...
        'energy': brain.energy,  # Guardar energía metabólica
        'symbol_genome': brain.symbol_genome,  # Guardar símbolos consolidados
...
def load_brain_state(filename):
    ...
    brain.energy = state.get('energy', 100.0)  # Cargar energía metabólica
    brain.symbol_genome = state.get('symbol_genome', np.empty((0, 9)))  # Cargar símbolos
    ...
    print(f"      Neuronas: {len(brain.genome)} activas | Símbolos: {len(brain.symbol_genome)}")
    print(f"      Umbral decisión: {brain.decision_threshold:.2f}")
    print(f"      Energía: {brain.energy:.1f}/{brain.MAX_ENERGY}")
...
def run_evolution(generations=5, population_size=10, candles_limit=100000): 
    ...
    for gen in range(generations):
        ...
        # Print estadísticas de esta generación
        if gen_best_brain is not None:
            ...
            print(f"🏆 MEJOR DE GEN {gen}: Balance ${gen_best_fitness:.2f} | Neuronas: {total_neurons} | Símbolos: {total_symbols} | Umbral: {umbral_display:.2f}")
            print(f"   🌱 Nacimientos: {novelty_births} por novedad, {mitosis_births} por mitosis")
        else:
...
if __name__ == "__main__":
    ...
    winner_brain = run_evolution(generations=5, population_size=50, candles_limit=100000)

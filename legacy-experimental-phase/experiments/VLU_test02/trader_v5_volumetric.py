...
class VolumetricNeuron3D:
    ...
    def __init__(self, genome_slice):
        ...
        # FIX B: Confianza ciega inicial (10.0)
        self.alpha = 10.0
        self.beta = 1.0
        self.confidence = 0.5
    ...
    def activate_3d_scan(self, market_trajectory, current_temp):
        ...
        # --- FASE 5: PONDERACIÓN (Ecuación 8: wi * phi) ---
        # phi = max_overlap (función de pertenencia volumétrica)
        # wi = weight (peso genético)
        # Confianza Bayesiana modula el peso efectivo
        self.confidence = self.alpha / (self.alpha + self.beta)
        weight = self.params[4]
        
        # Ecuación 8: wi * phi(x) * confidence
...
class VolumetricBrain:
    ...
    def predict(self, market_point, current_volatility):
        """
        Predicción volumétrica con MAX-AGGREGATION (Ecuación 8).
        
        En lugar de sumar activaciones, seleccionamos la neurona con
        mayor |wi * phi| (el "clic" geométrico más fuerte).
        ...
        # MAX-AGGREGATION: Encontrar neurona con mayor activación absoluta
        max_activation = 0.0
        ...
        for neuron in self.neurons:
            ...
            # Seleccionar por valor absoluto (magnitud del encaje)
            if abs(activation) > abs(max_activation):
                max_activation = activation
                winner_activation = activation
        
        # Retornar señal del ganador (mantiene signo para LONG/SHORT)
...
def prepare_inputs_3d(prices, volumes, volatilities):
    """
    Prepara inputs para espacio de fases 3D con NORMALIZACIÓN ROLLING.
    
    CRÍTICO: Evita look-ahead bias usando solo datos pasados para normalizar.
    ...
    # NORMALIZACIÓN ROLLING (ventana de 100 períodos)
    # Evita usar información del futuro
    window = 100
    ...
    for i in range(len(returns)):
        ...
        # Normalizar retornos con media/std de ventana pasada

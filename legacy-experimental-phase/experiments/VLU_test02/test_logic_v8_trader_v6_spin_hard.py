...
def fetch_extended_data(symbol="PEPEUSDT", interval="1m", total_candles=50000):
    """
...
def run_evolution(generations=100000): # Poner 10000+ para serio
    ...
    market_prices, market_vols, market_volatilities = fetch_extended_data("PEPEUSDT", "1m", 50000)
    ...
    for gen in range(generations):
        ...
        for t in range(len(inputs)):
            ...
            if (signal > 0.8 and position == -1) or (signal < -0.8 and position == 1):
                ...
                # --- CORRECCIÓN W: FEE SOBRE NOTIONAL ---
                # El fee se paga sobre el volumen total movido (Apuesta x Palanca)
            ...
            if position == 0:
                if signal > 0.8: # LONG
                    position = 1
                    # Slippage afecta al precio de entrada
                    entry_price = current_price * (1 + SLIPPAGE) 
                    current_bet = min(balance, MAX_BET)
                    
                    # --- CORRECCIÓN W: FEE DE ENTRADA ---

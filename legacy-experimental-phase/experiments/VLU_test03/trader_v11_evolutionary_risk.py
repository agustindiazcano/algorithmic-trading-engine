...
from risk_fall_sense import FallSenseController, FallSenseConfig, SuggestedAction

# ==============================================================================
# 🧠 CEREBRO CAUSAL DINÁMICO (V9 + V10 Spin Dynamics + Fall Sense)
# ==============================================================================

class DynamicCausalBrain:
    def __init__(self, n_neurons=10, A_dc=1.618):
        # --- Fall Sense Controller (V10) ---
        self.fall_sense = FallSenseController()
    ...
    def reset_genome(self):
        # Genoma EXPANDIDO: [x, y, z, R, W, Sx, Sy, Sz, ÁNGULO, LONGITUD, OMEGA_0, MASA, SIGMA]
        # 13 genes totales
        self.genome = []
        for _ in range(self.n_neurons):
            gene = np.concatenate([
                ...
                [np.random.uniform(-np.pi/2, np.pi/2)], # Theta (ángulo de spin)
                [np.random.randint(5, 60)],          # L (longitud de memoria)
                [np.random.uniform(0.1, 10.0)],      # Omega_0 (frecuencia natural)
                [np.random.uniform(0.1, 5.0)],       # m (masa inercial)
                [np.random.choice([-1.0, 1.0])],     # Sigma (dirección de spin: -1=Bear, +1=Bull)
                
                # --- GENES DE RIESGO (V11) ---
                [np.random.uniform(0.0, 10.0)],      # K_MAE (Pánico al rojo)
                [np.random.uniform(0.0, 20.0)],      # K_SPREAD (Odio al spread)
                [np.random.uniform(0.5, 5.0)]        # K_LIQ (Miedo a la muerte)
            ])
    ...
    def predict(self, history_points):
        # Inferencia Volumétrica SECUENCIAL CON SPIN 2D + MOMENTUM 🌪️⚡
        if len(self.genome) == 0: return 0.0
        
        # 1. Desempaquetar (13 genes)
        ...
        # NUEVOS GENES FÍSICOS
        omega_0 = self.genome[:, 10]      # Frecuencia natural
        mass = self.genome[:, 11]         # Masa inercial
        mass = self.genome[:, 11]         # Masa inercial
        sigma = self.genome[:, 12]        # Dirección de spin (-1 o +1)
        
        # Genes de Riesgo (V11) - Tomamos el promedio (democracia neuronal) o el del líder?
        # Por seguridad y consistencia, usaremos el PROMEDIO de la población neuronal para configurar el "cuerpo".
        k_mae = np.mean(self.genome[:, 13])
        k_spread = np.mean(self.genome[:, 14])
        k_liq = np.mean(self.genome[:, 15])
        
        # Actualizar Configuración de FallSense en tiempo real (si cambió)
        self.fall_sense.config.k_mae = k_mae
        self.fall_sense.config.k_spread = k_spread
        # Map K_LIQ logic roughly to existing params or add new one? 
        # Original logic uses k_stop_risk etc. Let's map:
        # K_LIQ -> k_stop_risk (closest proxy for death proximity sensitivity)
        self.fall_sense.config.k_stop_risk = k_liq 
        ...
        # Track winner for detailed stats
        ...
        for i in range(n):
            ...
            # --- DINÁMICA (NUEVO) ---
            # Velocidad: cambio en precio (primera derivada)
            if len(neuron_history) >= 2:
                velocity = np.diff(neuron_history[:, 0])  # Precio es columna 0
                avg_velocity = np.mean(np.abs(velocity))
            else:
                avg_velocity = 0.0
            
            # Aceleración: cambio en velocidad (segunda derivada)
            if len(neuron_history) >= 3:
                accel = np.diff(velocity)
                avg_accel = np.mean(np.abs(accel))
            else:
                avg_accel = 0.0
            
            # Velocidad angular: qué tan rápido gira el patrón
            omega_actual = avg_velocity / (radii[i] + 1e-8)
            
            # Resonancia: ¿coincide la frecuencia del mercado con la natural de la neurona?
            resonance = np.exp(-((omega_actual - omega_0[i])**2) / (omega_0[i]**2 + 1e-8))
            
            # Inercia: fuerza basada en volumen
            if len(neuron_history) >= 1:
                avg_volume = np.mean(neuron_history[:, 1])  # Volumen es columna 1
                force = avg_volume * avg_accel
                inertia_factor = force / (mass[i] + 1e-8)
            else:
                inertia_factor = 0.0
            
            # Alineación de spin: ¿va en la dirección correcta?
            if len(neuron_history) > 1:
                price_direction = np.sign(neuron_history[-1, 0] - neuron_history[0, 0])
            else:
                price_direction = 0
            
            spin_alignment = 1.0 if price_direction == sigma[i] else 0.5  # Penalización si va al revés
            
            # Momentum compuesto
            momentum = resonance * (1.0 + inertia_factor) * spin_alignment
            momentum = np.clip(momentum, 0.0, 2.0)  # Cap en 2x boost
            
            # --- LAGRANGIAN (NUEVO) ---
            # Energía Cinética: T = 0.5 * m * v^2
            # Usamos avg_velocity como v
            T = 0.5 * mass[i] * (avg_velocity ** 2)

            # Energía Potencial: V = k * d^2 (como un resorte)
            # Usamos min_dist_sq como d^2 (ya es cuadrada)
            k_spring = 1.0  # Constante de resorte (fija por ahora)
            V = k_spring * min_dist_sq

            # Lagrangiano: L = T - V
            lagrangian = T - V
            total_lagrangian += lagrangian

            # Solo activar FUERTE si L > 0 (energía cinética supera potencial)
            # Si L < 0, penalizamos la activación
            lagrangian_factor = 1.0 if lagrangian > 0 else 0.3
            
            # Activación final con momentum y lagrangian
            current_signal = activation * weights[i] * confidence[i] * momentum * lagrangian_factor
            total_activation += current_signal
            ...
            # Capture winner stats
            if abs(current_signal) > max_signal_strength:
                max_signal_strength = abs(current_signal)
                winner_stats = {
                    "mass": mass[i],
                    "omega": omega_0[i],
                    "spin": sigma[i],
        ...
        self.last_stats = {
            ...
            "momentum": total_momentum / (n + 1e-9),  
            "lagrangian": total_lagrangian / (n + 1e-9) 
        }
        # Add winner stats
        self.last_stats.update(winner_stats)
...
def run_evolution(generations=100, population_size=20): # <-- Population Size
    ...
    for gen in range(generations):
        ...
        for _ in range(population_size):
            ...
            # Constraints for geometry genes
            mutant[:, 3] = np.abs(mutant[:, 3])     # Radios positivos
            mutant[:, 5:8] = np.abs(mutant[:, 5:8]) # Stretch positivo
            mutant[:, 9] = np.clip(np.abs(mutant[:, 9]), 5, 60).astype(int)  # Length [5, 60]
            
            # NEW: Constraints for physical genes
            mutant[:, 10] = np.clip(np.abs(mutant[:, 10]), 0.1, 10.0)  # omega_0 [0.1, 10.0]
            mutant[:, 11] = np.clip(np.abs(mutant[:, 11]), 0.1, 5.0)   # mass [0.1, 5.0]
            mutant[:, 12] = np.sign(mutant[:, 12])  # sigma in {-1, +1}
            
            # Constraints for Risk Genes (V11)
            mutant[:, 13] = np.clip(np.abs(mutant[:, 13]), 0.0, 10.0) # K_MAE
            mutant[:, 14] = np.clip(np.abs(mutant[:, 14]), 0.0, 20.0) # K_SPREAD
            mutant[:, 15] = np.clip(np.abs(mutant[:, 15]), 0.5, 5.0)  # K_LIQ
            ...
            for t in range(len(inputs)):
                ...
                if (signal > mutant_threshold and position == -1) or \
                   (signal < -mutant_threshold and position == 1):
                    ...
                    if profit_usd > 0: current_winners += 1
                    else: current_losers += 1
                    
                    # 🧠 FEEDBACK
                    brain.feedback(profit_usd)
                ...
                if position != 0:
                    ...
                    if unrealized_pnl <= -0.9: # Margin Call
                        balance -= current_bet
                        current_losers += 1 # LIQUIDATION IS A LOSS
                        position = 0
                        current_bet = 0
            ...
            if position != 0:
                ...
                # Update stats for final close? Not strictly needed for fitness but good for logging.
                if profit_usd > 0: current_winners += 1
                else: current_losers += 1
            ...
            if fitness > best_of_batch_fitness:
                ...
                # Extract Risk Genes for logging
                k_mae = np.mean(mutant[:, 13])
                k_spr = np.mean(mutant[:, 14])
                k_liq = np.mean(mutant[:, 15])
                ...
                best_of_batch_stats = {
                    ...
                    'k_mae': k_mae, 'k_spr': k_spr, 'k_liq': k_liq
                }
        ...
        if best_of_batch_fitness > best_fitness:
            ...
            # 🏆 LOG DETALLADO
            _win = best_of_batch_stats.get('winners', 0)
            _los = best_of_batch_stats.get('losers', 0)
            _wr = (_win / (_win + _los + 1e-9)) * 100
            
            print(f"🏆 [Gen {gen}] NEW CHAMP! Profit: ${best_of_batch_stats['profit']:+.2f} | Trades: {best_of_batch_stats['trades']} (W:{_win} L:{_los} WR:{_wr:.1f}%)")
            print(f"   🧬 RISK GENES: K_MAE={best_of_batch_stats['k_mae']:.1f} | K_SPR={best_of_batch_stats['k_spr']:.1f} | K_LIQ={best_of_batch_stats['k_liq']:.1f} | Thr: {best_threshold:.3f}")
...
def run_simulation_test(genome, threshold, inputs, prices, TRADING_MODE="BOTH", max_bet=None):
    """Simulación pura sin gráficos ni outputs, devuelve métricas."""
    brain = DynamicCausalBrain(n_neurons=len(genome), A_dc=1.618)
    brain.genome = genome
    
    INITIAL_BALANCE = 10000.0
    # Si no pasaron max_bet, usar el default global o el valor 3333.0
    if max_bet is None:
        max_bet = 3333.0
    SLIPPAGE = 0.001
    FEE = 0.001
    ...
    # Init Fall Sense
    fall_sense = FallSenseController() # <-- Inicializar Controlador
    ...
    for t in range(len(inputs)):
        ...
        pos_side = "LONG" if position == 1 else ("SHORT" if position == -1 else "FLAT")
        stop_price = None # No hard stop used here, reliant on FallSense
        
        # Approximate MAE (simple distance from entry)
        mae = 0.0
        if position == 1: mae = max(0, (entry_price - current_price)/entry_price)
        elif position == -1: mae = max(0, (current_price - entry_price)/entry_price)
        
        fall_state = fall_sense.update(
            mark_price=current_price,
            liquidation_price=liq_price,
            position_side=pos_side,
            stop_price=stop_price,
            mae_pct=mae,
            spread_pct=0.0001, # Assumed low spread in backtest
            leverage=LEVERAGE
        )
        last_fall_state = fall_state
        
        # ⚡ REFLEJOS ROBÓTICOS (Priority Over Signals)
        reflex_action = fall_state.action
        
        if reflex_action == SuggestedAction.EXIT_NOW and position != 0:
            # FORCE EXIT
            pnl_pct = 0
            ...
            trade_log.append({
                ...
                "exit_time_idx": t,
                "compatibility": entry_stats.get("compatibility", 0) * 100,
                "confidence": entry_stats.get("confidence", 0) * 100,
                "momentum": entry_stats.get("momentum", 0) * 100,     # <-- Added
                "lagrangian": entry_stats.get("lagrangian", 0) * 100, # <-- Added
                "fall_score": fall_state.fall_score * 100,  # Log Fall Score
                "fall_action": fall_state.action.name
            })
            
            position = 0
            current_bet = 0
            fall_sense.reset() # Reset logic after exit
        ...
        if (signal > threshold and position == -1) or \
           (signal < -threshold and position == 1):
            ...
            trade_log.append({
                ...
                "compatibility": entry_stats.get("compatibility", 0) * 100,
                "confidence": entry_stats.get("confidence", 0) * 100,
                "momentum": entry_stats.get("momentum", 0) * 100,
                "lagrangian": entry_stats.get("lagrangian", 0) * 100,
                "w_mass": entry_stats.get("mass", 0),
                "w_omega": entry_stats.get("omega", 0),
                "w_spin": entry_stats.get("spin", 0),
                "w_spin": entry_stats.get("spin", 0),
                "w_veloc": entry_stats.get("velocity", 0),
                "fall_score": fall_state.fall_score * 100 # Log Fall Score on normal exit too
            })
        ...
        if position == 0:
            ...
            # V10 GATE: Can we trade?
            if fall_state.action in [SuggestedAction.NO_NEW_TRADES, SuggestedAction.EXIT_NOW, SuggestedAction.COOLDOWN]:
                # 🚫 ROBOT SAYS NO
                continue
                
            # Scale bet based on Fall Score (Stability)
            scale_factor = fall_state.scale_factor
...
if __name__ == "__main__":
    ...
    # Ajustá 'generations' (tiempo) y 'population_size' (variedad/individuos)
    winner, winner_thr, prices, vols, inputs, times, best_log = run_evolution(generations=100, population_size=20)
    ...
    if winner is not None:
        ...
        np.save(f"best_genome_spin_dynamics_{ts}.npy", winner)
        np.save(f"best_threshold_spin_dynamics_{ts}.npy", winner_thr)
        
        # Guardar tambien como 'latest' para facilitar el test rápido
        np.save("best_genome_spin_dynamics.npy", winner)
        np.save("best_threshold_spin_dynamics.npy", winner_thr)
        
        print(f"   [+] Archivo guardado: best_genome_spin_dynamics_{ts}.npy (y latest)")
        ...
        for i in range(1, 11):
            ...
            try:
                ...
                # 3. Correr Test
                bal, trades, acts, _ = run_simulation_test(winner, winner_thr, t_inputs, t_prices_aligned, TRADING_MODE="BOTH")
        ...
        # Volver a correr run_simulation_test con los inputs originales del entrenamiento
        bal_final, tr_final, acts_final, _ = run_simulation_test(winner, winner_thr, inputs, prices, TRADING_MODE="BOTH")

...
# 🔒 CONSTANTES UNIVERSALES (Leyes de Di-Ca 2.0)
K_UNIVERSAL = 0.2816
...
def calculate_higuchi_df(series, k_max=10):
    ...
    N = len(series)
    if N < 9: return 1.5 # Valor neutro si no hay data
    L = []
    x = np.array(series)
...
def run_scientific_experiment(df_full):
    ...
    for i in range(9):
        ...
        print(f" > Test {i+1:02} | Bal: ${final_bal:8.2f} | WR: {wr:5.1f}% ({wins}/{total}) | Df Prom: {df_window['df_vascular'].mean():.2f}")

...
import random
import sys
import trader_v9_spin_dynamics
from trader_v9_spin_dynamics import DynamicCausalBrain, fetch_extended_data, run_simulation_test
...
def main():
    ...
    try:
        ...
        trader_v9_spin_dynamics.INITIAL_BALANCE = CAPITAL_INICIAL
        trader_v9_spin_dynamics.MAX_BET = CAPITAL_INICIAL * RIESGO_POR_TRADE 
        print(f"   💰 Capital Test: ${CAPITAL_INICIAL:,.0f} | Apuesta Máx: ${trader_v9_spin_dynamics.MAX_BET:,.0f} ({RIESGO_POR_TRADE*100}%)")
 ...
 # 0.65 
    ...
    for i in range(1, 11):
        ...
        try:
            ...
            # Correr Simulación
            # Correr Simulación
            bal, trades, actions, trade_log = run_simulation_test(winner, winner_thr, inputs, prices_aligned, TRADING_MODE="BOTH", max_bet=trader_v9_spin_dynamics.MAX_BET)
            ...
            if trade_log:
                ...
                print("   " + "-"*90)
                print(f"   {'TIPO':<6} | {'ENTRADA':<10} | {'SALIDA':<10} | {'PNL %':<8} | {'PNL $':<10} | {'CONF':<4} | {'COMP':<4} | {'MOM':<4} | {'LAG':<5} | {'MASS':<4} | {'OMEG':<4} | {'SPIN':<4} | {'VEL':<6}")
                print("   " + "-"*130)
                for t in trade_log:
                    ...
                    mass = t.get('w_mass', 0)
                    omega = t.get('w_omega', 0)
                    spin = t.get('w_spin', 0)
                    veloc = t.get('w_veloc', 0) * 1000 # Escalar para que se vea
                    
                    print(f"   {t['type']:<6} | {t['entry_price']:<10.6f} | {t['exit_price']:<10.6f} | {pnl_color}{t['pnl_pct']:<7.2f}% | {pnl_color}${t['profit_usd']:<9.2f} | {t['confidence']:<4.0f} | {t['compatibility']:<4.0f} | {mom_val:<4.0f} | {lag_val:<5.0f} | {mass:<4.1f} | {omega:<4.1f} | {spin:<4.0f} | {veloc:<6.3f}")
                print("   " + "-"*130)

...
print("🧬 Cargando genoma PSO + CMA-ES...")
genome = BounceHunterGenome.load("pso_cma_best.json")

print(f"\n📊 Parámetros del genoma:")
print(f"  Radio Base: {genome.radius_base:.3f}")
print(f"  Inercia Up: {genome.inertia_up:.3f}")
print(f"  Inercia Down: {genome.inertia_down:.3f}")
...
print(f"✅ {len(prices)} velas cargadas")
...
# Stats
print("\n" + "="*60)
print("📊 RESULTADOS - PSO + CMA-ES GENOME")
print("="*60)
...
ax1.set_title('PSO + CMA-ES Evolved VNN', fontsize=14, fontweight='bold')
...
# 2. Equity
ax2 = axes[1]
ax2.plot(vnn.equity_curve, color='#00ff00', linewidth=2, label=f'PSO+CMA VNN (${vnn.equity_curve[-1]:,.0f})')
...
plt.tight_layout()
plt.savefig('pso_cma_backtest.png', dpi=150, bbox_inches='tight')
print("\n📊 Gráfico guardado: pso_cma_backtest.png")
